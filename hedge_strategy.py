...
class HedgeStrategy:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.order_timeout_seconds = config.get("order_timeout_seconds", 60)  # Time before canceling order
        self.max_retries = config.get("max_retries", 3)  # Maximum number of order placement retries
    ...
    async def shutdown(self):
        ...
        try:
            ...
            self.logger.log("Disconnected from exchanges", "INFO")
        except Exception as e:
    ...
    async def place_lighter_limit_order(self):
        ...
        try:
            ...
            if order_result.success:
                ...
                self.logger.log(f"Lighter order placed successfully: {self.lighter_order_id}", "INFO")
                return True
            else:
    ...
    async def monitor_lighter_order(self):
        ...
        # Register for order updates via WebSocket if available
        try:
            # Set up a callback for order updates
            self.lighter_client.register_order_update_callback(self.on_lighter_order_update)
        except Exception as e:
            self.logger.log(f"Failed to register for WebSocket updates, falling back to polling: {e}", "WARNING")
        ...
        while self.is_running and not self.lighter_order_filled:
            try:
                ...
                if self.order_placement_time and (current_time - self.order_placement_time > self.order_timeout_seconds):
                    self.logger.log(f"Order timeout reached ({self.order_timeout_seconds}s). Canceling order.", "WARNING")
                    await self.cancel_lighter_order()
                    
                    # Check if we should retry
                    if self.current_retry_count < self.max_retries:
                        self.current_retry_count += 1
                        self.logger.log(f"Retrying order placement (attempt {self.current_retry_count}/{self.max_retries})", "INFO")
                ...
                # Get active orders from Lighter
                ...
                # If not found in active orders, check position/info to infer fill
                if not found:
                    info = await self.lighter_client.get_order_info(self.lighter_order_id)
                    if info and info.status == "FILLED":
                        self.lighter_order_filled = True
                        self.logger.log(f"Lighter order {self.lighter_order_id} filled (by position)", "INFO")
                        return True
    ...
    async def run(self):
        ...
        try:
            ...
            if not await self.monitor_lighter_order():
            ...
            if not await self.place_paradex_hedge_order():
